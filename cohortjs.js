...
async function viewClusterStats(clusterId, clusterName) {
    ...
    try {
        // NOTE: This assumes an existing server-side API endpoint for stats.
...
async function checkAllClusterStatuses() {
    ...
    const statusChecks = dynamicClusters.map(async (category) => {
        ...
        for (let i = 0; i < MAX_RETRIES; i++) {
            try {
                // NOTE: This assumes an existing server-side API endpoint for cohort status.
...
async function handleFormSubmission(event) {
    ...
    try {
        // NOTE: This assumes an existing server-side API endpoint for joining a cluster.
...
async function downloadContacts(fileName, clusterId) { 
    ...
    try {
        ...
        // NOTE: This assumes an existing server-side API endpoint for downloading contacts.
        ...
        if (contentType && contentType.includes('text/vcard')) {
            ...
            try {
                 // NOTE: This assumes an existing server-side API endpoint for tracking downloads.
...
async function initializeSupabase() {
    ...
    // Check for the Supabase library being loaded via CDN in the HTML
    if (!window.supabase || !window.supabase.createClient) {
         console.error("FATAL: Supabase library not found on global window object. Ensure the CDN script tag is present in HTML.");
         isAuthReady = true;
         renderClusters(); 
         return;
    }

    // --- CRITICAL CONFIGURATION CHECK FOR SERVER INJECTION FAILURE ---
    if (supabaseUrl.includes('__SUPABASE_URL_INJECTION__') || supabaseAnonKey.includes('__SUPABASE_ANON_KEY_INJECTION__')) {
        const errorMessage = "FATAL ERROR: Supabase configuration placeholders were not replaced by the server. Check your server-side template serving logic.";
    ...
    try {
        ...
        if (session && session.user) {
            ...
            // Fetch profile details which sets userCountry and userReferralCode
...
// Expose required functions to the global scope for onclick handlers in the HTML
