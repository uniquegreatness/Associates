...
/**
 * Endpoint 2: Get Cluster Status (Membership, VCF Upload State, AND USER PROFILE)
 * FIX: Enhanced to include the requesting user's profile data for the dashboard.
 */
router.get('/cohorts/:cluster_id/status', async (req, res) => {
    ...
    // 1. Get Cluster Status Metrics
    const result = await getCohortStatus(clusterIdNum, user_id);
    
    // 2. Fetch User's Public Profile (Essential for Dashboard)
    // We fetch only non-sensitive public fields the client needs to display "Your Status"
    try {
        const { data: profile, error: profileError } = await supabase
            .from('profiles')
            .select('nickname, age, gender, country, profession, display_profession, friend_reasons, services')
            .eq('user_id', user_id)
            .maybeSingle(); // Use maybeSingle for clean handling of missing profile

        if (profileError) {
            // Log warning but continue; a missing profile shouldn't crash the app
            console.warn(`Warning: Could not fetch public profile for user ${user_id}:`, profileError.message);
        }
        
        // Attach profile data to the result object
        result.user_profile = profile || null;

    } catch (e) {
         console.error(`Error fetching user profile for status endpoint: ${e.message}`);
         // Fail gracefully if profile fetch fails
