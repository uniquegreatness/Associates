...
// This now uses the PostgreSQL function for atomic table management.
// ----------------------------------------------------
app.post('/api/join-cluster', async (req, res) => {
    ...
    try {
        ...
        // This function handles table creation, profile fetch, insertion, and capacity check.
        ...
        if (rpcError) {
            ...
            // Check for specific error messages raised by the function (like unique violation)
        ...
        if (cohortStatus.is_full) {
            ...
            // NOTE: We must fetch the data before the critical delete step.
            ...
            const { error: uploadError } = await supabase.storage
                .from('near_vcf_bucket') // IMPORTANT: Check your actual bucket name
            ...
            if (uploadError) {
                ...
                // CRITICAL: Log failure, but we must still try to delete raw data.
            }
            ...
            if (deleteError) {
                ...
                // Alert Admin here, but return success to user since VCF is generated.
            } else {
