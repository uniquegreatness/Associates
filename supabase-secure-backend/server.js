...
/**
 * 1. Retrieves the current status (cohort_id, members, is_full) for a given cluster.
 * 2. Checks if the specific user is a member of the active cohort.
 * @param {number} cluster_id - The ID of the interest cluster (1-10).
 * @param {string} user_id - The user's Supabase ID.
 * @returns {Promise<{success: boolean, message: string, cohort_id?: string, is_full?: boolean, current_members?: number, user_is_member?: boolean}>}
 */
async function getCohortStatus(cluster_id, user_id) {
    ...
    try {
        // Step 1: Find the currently active cohort for this cluster
        ...
        let cohort_id, current_members, is_full, user_is_member = false;

        if (cohortError) throw cohortError;

        if (cohortData) {
            cohort_id = cohortData.cohort_id;
            current_members = cohortData.current_members;
            is_full = cohortData.is_full;
            
            // Step 2: Check if the user is in this active cohort
            ...
            user_is_member = memberCount > 0;

        } else {
            // No active cohort found (meaning the category is ready for a new one)
            cohort_id = `C_OPEN_${cluster_id}`; // Mock ID for open state
            current_members = 0;
            is_full = false;
        }
...
// --- NEW REDIRECT SCRIPT FOR CENTRALIZED INJECTION (Unchanged) ---
...
// ----------------------------------------------------
// SECURE API ENDPOINT: JOIN CLUSTER (RPC Call) - ORIGINAL LOGIC PRESERVED
// ----------------------------------------------------
app.post('/api/join-cluster', async (req, res) => {
    ...
    try {
        ...
        // ✅ FIX: Using the confirmed function name 'complete_cohort_exchange'.
        ...
        if (cohortStatus.is_full) {
            ...
            // NOTE: Using the same function name, but only passing p_cohort_id.
            // This relies on the function being overloaded or capable of handling different numbers of arguments.
