...
const { createClient, raw } = require('@supabase/supabase-js'); // ADDED 'raw'
...
app.post('/api/track-download', async (req, res) => {
    ...
    try {
        ...
        // 2. Aggregate Update: Atomically Increment vcf_download_count
// CRITICAL FIX: Using a simple string for atomic increment on a column.
const { error: countUpdateError } = await supabase
    .from('cluster_metadata')
    .update({ 
        vcf_download_count: raw('vcf_download_count + 1'), // Use the imported 'raw' function
    ...
    .eq('cluster_id', clusterIdNum);
        ...
        if (countUpdateError) {
             ...
             console.log(`Successfully tracked download for user ${user_id.substring(0, 8)}... on cluster ${clusterIdNum}.`);
        }
