...
async function getCohortStatus(cluster_id, user_id) {
    ...
    try {
        ...
        const { data: clusterMeta, error: metaError } = await supabase
            ...
            // UPDATED: Include vcf_downloads_count
            .select('active_cohort_id, vcf_uploaded, vcf_file_name, max_members, cluster_name, vcf_downloads_count') 
        ...
        // NEW: Capture download count
        const vcf_downloads_count = clusterMeta.vcf_downloads_count || 0;
        ...
        return {
            ...
            max_members,
            cluster_name,
            // UPDATED: Return the downloads count
            vcf_downloads_count, 
        ...
        if (activeCohortId.startsWith('C_OPEN_')) {
            ...
            const { error: metaUpdateError } = await supabase
                ...
                .update({ 
                    ...
                    vcf_uploaded: false,
                    vcf_file_name: null,
                    // Reset downloads count when a new cohort starts
                    vcf_downloads_count: 0 
                }) 
                .eq('cluster_id', clusterIdNum);
    ...
    try {
        ...
        const { error: statusUpdateError } = await supabase
            ...
            .update({ 
                ...
                // Reset downloads count for the new open state
                vcf_downloads_count: 0 
            }) 
            .eq('cluster_id', clusterIdNum); 
    ...
    try {
        ...
        if (clusterIdToUpdate) {
            ...
            if (fetchError || !current) {
                ...
                const newCount = (current.vcf_downloads_count || 0) + 1;
                
                // 2. Update count
                const { error: updateError } = await supabase
                    .from('cluster_metadata')
                    .update({ vcf_downloads_count: newCount })
                    .eq('cluster_id', clusterIdToUpdate);
