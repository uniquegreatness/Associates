...
// --- REWRITTEN DATABASE SERVICE FUNCTION: GET COHORT STATUS (Confirmed Fix for vcf_download_count) ---
...
async function getCohortStatus(cluster_id, user_id) {
    ...
    try {
        ...
        const { data: clusterMeta, error: metaError } = await supabase
            ...
            // FIX CONFIRMED: Using singular 'vcf_download_count'
            .select('active_cohort_id, vcf_uploaded, vcf_file_name, max_members, cluster_name, vcf_download_count') 
        ...
        if (!clusterMeta) {
            // THIS IS THE ERROR SOURCE: Row is missing in cluster_metadata
        ...
        // FIX CONFIRMED: Retrieving from singular 'vcf_download_count'
        ...
        return {
            ...
            max_members,
            cluster_name,
            // UPDATED: Return the downloads count (key name remains plural for consistency with front end if needed)
            vcf_downloads_count, 
...
// ----------------------------------------------------
// REWRITTEN SECURE API ENDPOINT: JOIN CLUSTER (Confirmed Fix for vcf_download_count)
// ----------------------------------------------------
        ...
        if (activeCohortId.startsWith('C_OPEN_')) {
            ...
            const { error: metaUpdateError } = await supabase
                ...
                .update({ 
                    ...
                    vcf_uploaded: false,
                    vcf_file_name: null,
                    // FIX CONFIRMED: Resetting with singular 'vcf_download_count'
                    vcf_download_count: 0 
                }) 
                .eq('cluster_id', clusterIdNum);
            ...
            if (vcfUploadSuccessful) {
                ...
                const { error: statusUpdateError } = await supabase
                    ...
                    .update({ 
                        ...
                        // FIX CONFIRMED: Initializing download count with singular 'vcf_download_count'
                        vcf_download_count: 0
                    }) 
                    .eq('cluster_id', clusterIdNum); 
...
// ----------------------------------------------------
// API ENDPOINT: DOWNLOAD VCF (Confirmed Fix for vcf_download_count)
// ----------------------------------------------------
app.get('/api/download-contacts', async (req, res) => {
    ...
    try {
        ...
        // --- NEW LOGIC: Increment VCF Download Count ---
        const clusterIdToUpdate = extractClusterIdFromFileName(fileName);

        if (clusterIdToUpdate) {
            // NOTE: This uses a read-then-update pattern which is susceptible to race conditions
            // but is the simplest method without introducing a database RPC function.
            
            // 1. Get current count
            const { data: current, error: fetchError } = await supabase
                .from('cluster_metadata')
                // FIX CONFIRMED: Selecting from singular 'vcf_download_count'
                .select('vcf_download_count')
                .eq('cluster_id', clusterIdToUpdate)
                .single();

            if (fetchError || !current) {
                console.error(`Warning: Failed to fetch current download count for cluster ${clusterIdToUpdate}.`, fetchError?.message);
            } else {
                // FIX CONFIRMED: Reading from singular 'vcf_download_count'
                const newCount = (current.vcf_download_count || 0) + 1;
                
                // 2. Update count
                const { error: updateError } = await supabase
                    .from('cluster_metadata')
                    // FIX CONFIRMED: Updating singular 'vcf_download_count'
                    .update({ vcf_download_count: newCount })
                    .eq('cluster_id', clusterIdToUpdate);

                if (updateError) {
                    console.error(`Warning: Failed to update download count to ${newCount} for cluster ${clusterIdToUpdate}.`, updateError.message);
                } else {
                    console.log(`Successfully incremented download count for Cluster ${clusterIdToUpdate} to ${newCount}.`);
                }
            }
        } else {
             console.warn(`Could not extract Cluster ID from file name: ${fileName}. Skipping download count update.`);
        }
        // --- END NEW LOGIC ---
