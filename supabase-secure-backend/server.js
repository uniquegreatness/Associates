...
app.post('/api/join-cluster', async (req, res) => {
    ...
    try {
        ...
        if (isFullAfterJoin) {
            ...
            // 3a. Fetch all required contact data for the VCF
            ...
            if (fetchError || !allContacts || allContacts.length === 0) {
                ...
                // Flatten the data structure for VCF generation utility
                const vcfContacts = allContacts.map(c => ({
                    ...c.user_profiles, // nickname, profession, whatsapp_number
                    display_profession: c.display_profession
                }));
                
                // 3b. Generate VCF Content
                ...
                // 3c. Upload VCF to Supabase Storage
            ...
            if (deleteError) {
                console.error(`CRITICAL FAILURE: Failed to delete raw data for ${newCohortId}`, deleteError);
            } else {
                 console.log(`Raw data for ${newCohortId} securely deleted and cluster opened.`);
            }
            ...
            const { error: statusUpdateError } = await supabase
                .from('cluster_metadata') 
                .update({ 
                    vcf_uploaded: true, // Set to true if VCF upload was successful or skipped
        ...
        return res.status(200).json({
            ...
            message: isFullAfterJoin ? `Success! Cohort ${newCohortId} is complete and VCF is generating.` : `Success! Joined cohort ${newCohortId}. Waiting for more members.`
        });
