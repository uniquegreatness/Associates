...
// CRITICAL: Admin password pulled from environment variables
...
// Ensure ADMIN_PASSWORD is set for the admin panel to function securely
if (!ADMIN_PASSWORD) {
    console.error("FATAL ERROR: ADMIN_PASSWORD environment variable is missing. Admin endpoints will not function securely.");
    // In production, you might want to exit here, but for development, we allow it to run.
}
...
// --- ADMIN SESSION MANAGEMENT (In-memory store) ---
// In a production environment with scaling, this should be replaced with Redis or a database store.
const ADMIN_SESSIONS = {};

/**
 * Middleware to check if the request has a valid admin session token in the cookie.
 */
function checkAdminAuth(req, res, next) {
    const token = req.cookies.admin_session_token;
    if (token && ADMIN_SESSIONS[token] && ADMIN_SESSIONS[token].expires > Date.now()) {
        // Token is valid and not expired
        next();
    } else {
        // Token is missing, expired, or invalid. Clear cookie if present.
        if (token && ADMIN_SESSIONS[token]) {
             delete ADMIN_SESSIONS[token]; // Clean up expired session
        }
        res.clearCookie('admin_session_token');
        res.status(401).json({ success: false, message: 'Authentication required. Session expired or invalid.' });
    }
}
// --- END ADMIN SESSION MANAGEMENT ---
...
function calculateClusterStats(members, userCountry) {
    ...
    if (abroadCount > 0) {
        ...
        geographic_mix['Abroad'] = Math.round(abardPercentage);
...
// --- HELPER FUNCTION: INJECT SUPABASE CONFIGURATION (Unchanged) ---
...
// --- FRONTEND SERVING CONFIGURATION (Updated with admin.html) ---
...
// --- NEW ROUTE: ADMIN PANEL PAGE ---
app.get('/admin.html', (req, res) => {
    // Serving the static admin_panel.html file directly
    const filePath = path.join(__dirname, '..', 'admin_panel.html');
    res.sendFile(filePath, (err) => {
        if (err) {
            console.error(`Error sending admin_panel.html: ${err.message}`);
...
// ----------------------------------------------------
// NEW SECURE ADMIN API ENDPOINTS (Login and CRUD)
// ----------------------------------------------------
...
app.post('/api/admin/login', (req, res) => {
    ...
    // Generate a secure, time-bound session token
    const token = crypto.randomUUID();
    const expiryTime = Date.now() + (1000 * 60 * 60 * 24); // 24 hour expiry

    ADMIN_SESSIONS[token] = { expires: expiryTime };

    // Set cookie for client-side persistence
    res.cookie('admin_session_token', token, {
        maxAge: expiryTime,
        httpOnly: true, // Recommended for security, but frontend uses it for check
...
// 3. ADMIN CRUD (Protected by checkAdminAuth middleware)
const ADMIN_CLUSTERS_TABLE = 'cluster_metadata';

// R: READ ALL Clusters
app.get('/api/admin/clusters', checkAdminAuth, async (req, res) => {
    try {
        const { data: clusters, error } = await supabase
            .from(ADMIN_CLUSTERS_TABLE)
            .select('*')
            .order('cluster_id', { ascending: true });

        if (error) throw error;
        res.status(200).json({ success: true, clusters });
    } catch (e) {
...
app.post('/api/admin/clusters', checkAdminAuth, async (req, res) => {
    const { cluster_name, region, max_members } = req.body;

    if (!cluster_name || !region || !max_members) {
        return res.status(400).json({ success: false, message: 'Missing required fields (name, region, capacity).' });
    }
    ...
    try {
        const newClusterData = {
            ...
            // Default statuses for new clusters
            vcf_uploaded: false,
        ...
        const { data, error } = await supabase
            .from(ADMIN_CLUSTERS_TABLE)
            .insert([newClusterData])
            .select();

        if (error) throw error;
        res.status(201).json({ success: true, cluster: data[0], message: 'Cluster created successfully.' });
    } catch (e) {
        console.error('Admin CREATE error:', e.message);
        res.status(500).json({ success: false, message: `Failed to create cluster: ${e.message}` });
    }
});

// U: UPDATE Existing Cluster
app.put('/api/admin/clusters/:id', checkAdminAuth, async (req, res) => {
    const clusterId = req.params.id;
    const updateData = req.body;

    try {
        // Ensure we only update valid fields. max_members must be an integer.
        if (updateData.max_members) {
            updateData.max_members = parseInt(updateData.max_members, 10);
        }
        ...
        res.status(200).json({ success: true, cluster: data[0], message: 'Cluster updated successfully.' });
    } catch (e) {
        console.error('Admin UPDATE error:', e.message);
        res.status(500).json({ success: false, message: `Failed to update cluster: ${e.message}` });
    }
});

// D: DELETE Cluster
app.delete('/api/admin/clusters/:id', checkAdminAuth, async (req, res) => {
    const clusterId = req.params.id;

    try {
        // NOTE: This will delete the metadata but *not* the associated members in other tables.
        // A foreign key constraint or separate cleanup might be necessary in a real system.
        ...
        // Check if a row was actually deleted (Supabase delete doesn't return data by default)
        // We'll rely on the error check for simplicity here.
        res.status(200).json({ success: true, message: `Cluster ID ${clusterId} deleted.` });
    } catch (e) {
        console.error('Admin DELETE error:', e.message);
        res.status(500).json({ success: false, message: `Failed to delete cluster: ${e.message}` });
    }
});

// ----------------------------------------------------
// END NEW SECURE ADMIN API ENDPOINTS
// ----------------------------------------------------
...
// NEW SECURE API ENDPOINT: GET CLUSTER STATS (THE MISSING ENDPOINT - FIXED)
...
// REWRITTEN SECURE API ENDPOINT: JOIN CLUSTER (PAUSE STATE IMPLEMENTED)
...
// NEW SECURE API ENDPOINT: RESET CLUSTER (Client-Triggered Cleanup)
...
// API ENDPOINT: DOWNLOAD VCF (Updated to use file_name)
