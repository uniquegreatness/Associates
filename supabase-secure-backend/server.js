...
// ----------------------------------------------------
// NEW SECURE API ENDPOINT: ADMIN LOGIN (Custom Auth for Admins)
// ----------------------------------------------------
app.post('/api/admin-login', async (req, res) => {
    ...
    // 1. Attempt to sign in using the standard Supabase client API
    ...
    if (signInError || !signInData.session) {
        // Return a generic authentication failure message for security
        console.warn(`Admin login attempt failed for user: ${email}. Error: ${signInError?.message}`);
    ...
    // 2. Perform Admin Role Check (Requires a column/table in your DB)
    // ASSUMPTION: You have an `is_admin` column in `user_profiles` or a separate `admin_users` table.
    // For this implementation, we will check `user_profiles.is_admin = true`.
    ...
    try {
        ...
        // Check if the user has the administrative role flag
        ...
        // 3. Set the required Supabase authentication cookies for the valid admin session
        const cookieOptions = { 
        ...
        console.error('Admin Role Check/Cookie Setup Error:', e.message);
        // Attempt to log out the user's session if a database error occurred after sign-in
        await supabase.auth.admin.signOut(user.id).catch(() => {});
        return res.status(500).json({ success: false, message: 'Internal server error during authentication process.' });
    }
});
