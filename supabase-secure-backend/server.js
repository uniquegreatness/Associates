...
// This is critical for serving all non-injected assets (CSS, JS, images)
...
// --- HELPER FUNCTION: INJECT SUPABASE CONFIGURATION (FIXED TEMPLATE PATHS) ---
function injectSupabaseConfig(templatePath, res) {
    // templatePath must be relative to the directory above server.js
...
// ++++++++++++++++ SURGICAL FIX: RESTORING FRONTEND ROUTES ++++++++++++++++

// --- FRONTEND SERVING CONFIGURATION (RESTORED ORIGINAL TEMPLATE NAMES) ---
...
app.get('/login.html', (req, res) => {
    injectSupabaseConfig('login_template.html', res); // FIXED: ensure correct template name
});

// Route for the leaderboard (secure dashboard) - USES INJECTION
app.get('/leaderboard.html', (req, res) => {
    injectSupabaseConfig('leaderboard_template.html', res); // FIXED: ensure correct template name
});

// Route for the new cohort page (cohort.html) - USES INJECTION
app.get('/cohort.html', (req, res) => {
    injectSupabaseConfig('cohort_template.html', res); // FIXED: ensure correct template name
});

// Dedicated page for password reset/update - USES INJECTION
app.get('/update-password.html', (req, res) => {
    injectSupabaseConfig('update-password_template.html', res); // FIXED: ensure correct template name
});

// 💡 NEW: Route for Admin Dashboard (must be injected)
app.get('/admin.html', (req, res) => {
    injectSupabaseConfig('admin_template.html', res); // FIXED: Using convention: admin_template.html
});

// ++++++++++++++++ SURGICAL FIX END: RESTORING FRONTEND ROUTES ++++++++++++++++
...
// ----------------------------------------------------
// 🔑 NEW API ENDPOINT: INITIATE PASSWORD RESET
// ----------------------------------------------------
app.post('/api/request-password-reset', async (req, res) => {
    const { email } = req.body;

    if (!email) {
        return res.status(400).json({ success: false, message: 'Email is required for password reset.' });
    }

    try {
        // Use the standard anonymous client to request the password reset link
        const { error } = await supabaseAnon.auth.resetPasswordForEmail(email, {
            redirectTo: `${req.protocol}://${req.get('host')}/update-password.html` 
        });
        ...
        // Successfully requested (or appears to have succeeded)
        res.status(200).json({ 
            success: true, 
            message: 'Password reset link sent! Check your admin email inbox.' 
        });

    } catch (e) {
        console.error('FATAL PASSWORD RESET ERROR:', e.message);
        // Generic success message even on potential failure to prevent email enumeration
        res.status(200).json({ 
            success: true, 
            message: 'Password reset link sent! Check your admin email inbox.' 
        });
...
// 3. UPDATE: Edit an Existing Cluster
app.put('/api/admin/clusters/:id', requireAdminAuth, async (req, res) => {
    ...
    const updates = {};
    if (cluster_name !== undefined) updates.cluster_name = cluster_name;
    if (cluster_region !== undefined) updates.cluster_region = cluster_region;

    // Validate and update max_members if provided
    if (max_members !== undefined) {
        const capacity = parseInt(max_members);
        if (isNaN(capacity) || capacity < 1) {
             return res.status(400).json({ success: false, message: 'Invalid capacity value.' });
        }
        updates.max_members = capacity;
    }
    ...
    try {
        ...
        if (error) throw error;
        if (!data) return res.status(404).json({ success: false, message: 'Cluster not found.' });

        res.status(200).json({ success: true, message: 'Cluster updated successfully.', cluster: data });
    } catch (e) {
...
app.get('/api/cluster-stats', async (req, res) => {
    ...
    try {
        ...
        const { data: cohortMembersRaw, error: membersFetchError } = await supabase
            .from('cluster_cohort_members')
...
app.post('/api/waitlist', async (req, res) => {
    ...
    try {
        ...
        // 1. Sign in the user immediately after creation.
        ...
        res.status(201).json({ 
            ...
            user_id: newUser.id,
            // IMPORTANT: Return the session data for the client to handle
            session: session 
        });
