...
// --- REFERRAL TRACKER MIDDLEWARE APPLICATION (CRITICAL PLACEMENT) ---
// This MUST be applied before routes that use req.cookies, like /api/waitlist
app.use(referralTracker); 
...
// ----------------------------------------------------
// EXISTING ROUTES (Waitlist and Leaderboard - Modified)
// ----------------------------------------------------
...
app.post('/api/waitlist', async (req, res) => {
    ...
    const referralCode = generateReferralCode(); // <--- NEW: Generate referral code (Logic 1)
    const profileToInsert = {
        user_id: newUser.id,
        email: email, 
        nickname: nickname,
        referrals: 0, 
        referral_code: referralCode, // <--- NEW: Store the generated code (Logic 1)
        ...otherProfileFields
    };
    ...
    try {
        ...
        // ----------------------------------------------------------------------
        // ✅ STEP 3: REFERRAL INCREMENT LOGIC (NEW - Logic 3)
        // ----------------------------------------------------------------------
        const referrerId = req.cookies['nearr_referrer_id'];
        
        if (referrerId && referrerId !== newUser.id) {
            console.log(`[Referral] New user ${newUser.id.substring(0, 8)}... referred by ${referrerId.substring(0, 8)}...`);
            
            // 1a. Read current referrals count
            ...
            if (fetchError || !currentReferrer) {
                console.error('[Referral] Failed to fetch current referral count. Skipping increment.', fetchError?.message);
            } else {
                const newReferrals = (currentReferrer.referrals || 0) + 1;
                
                // 1b. Update referrer's count (Read-then-update)
                ...
                if (updateError) {
                    console.error('[Referral] Failed to update referral count:', updateError.message);
                } else {
                    console.log(`[Referral] Referral count incremented for user ${referrerId.substring(0, 8)}... to ${newReferrals}.`);
                }
            }

            // 2. Clear the referral cookies as the referral has been processed
            res.clearCookie('nearr_referrer_id');
            res.clearCookie('nearr_referrer_code');

        } else if (referrerId && referrerId === newUser.id) {
             // Edge case: User refers themselves (clear cookies)
             res.clearCookie('nearr_referrer_id');
             res.clearCookie('nearr_referrer_code');
        }
        // ----------------------------------------------------------------------
        // END REFERRAL INCREMENT LOGIC
        // ----------------------------------------------------------------------

        
        // ----------------------------------------------------------------------
        // ✅ STEP 4: ESTABLISH ACTIVE SESSION 
        // ----------------------------------------------------------------------
...
// ----------------------------------------------------
// NEW FUNCTION DEFINITIONS (Referral Logic 1 & 2)
// Placed at the bottom to satisfy the user's request.
// ----------------------------------------------------

// --- NEW HELPER FUNCTION: Referral Code Generation (Logic 1) ---
function generateReferralCode() {
    // Generates an 8-character alphanumeric code using crypto for uniqueness
    return crypto.randomBytes(4).toString('hex').toUpperCase(); 
}
// --- END NEW HELPER FUNCTION ---

// --- NEW MIDDLEWARE: Referral Tracking (Logic 2) ---
async function referralTracker(req, res, next) {
    // Check if a referral code is present in the query parameters
    const referralCode = req.query.ref; 
    
    if (referralCode) {
        console.log(`[Referral] Found incoming code: ${referralCode}`);

        // Prioritize the *new* code, but shouldn't re-query if the code is the same
        if (req.cookies['nearr_referrer_code'] === referralCode.toUpperCase()) {
            console.log(`[Referral] Code ${referralCode} already tracked.`);
            return next();
        }

        try {
            // Use Supabase Anon client to query the user_profiles table for the referrer's ID
            const { data: referrer, error } = await supabaseAnon
                .from('user_profiles') 
                .select('user_id') 
            ...
            if (referrer) {
                ...
                // Set a temporary cookie to store the referrer's user_id until registration happens
                const cookieOptions = { 
                ...
                res.cookie('nearr_referrer_id', referrerId, cookieOptions);
                res.cookie('nearr_referrer_code', referralCode.toUpperCase(), cookieOptions);
                console.log(`[Referral] Successfully tracked referrer ID: ${referrerId.substring(0, 8)}...`);
            } else {
                console.warn(`[Referral] Code ${referralCode} not found. Clearing stale cookies.`);
                res.clearCookie('nearr_referrer_id');
                res.clearCookie('nearr_referrer_code');
            }
        } catch (e) {
            console.error('[Referral] Database query error during tracking:', e.message);
        }
    }
