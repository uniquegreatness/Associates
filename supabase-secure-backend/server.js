...
// ----------------------------------------------------
// API ENDPOINT: GET ALL CLUSTERS WITH USER DOWNLOAD STATUS (FIXED)
// ----------------------------------------------------
app.get('/api/clusters', async (req, res) => {
    const { user_id } = req.query;

    // Step 1: Fetch all clusters (All rows from cluster_metadata)
    ...
    if (allError) {
        console.error('Error fetching all cluster metadata:', allError.message);
        return res.status(500).json({ success: false, message: 'Failed to fetch cluster data.' });
    }
    
    // If no user_id is provided, return all clusters with the default status
    if (!user_id) {
    ...
    try {
        // Step 2: Fetch the user's download status for ALL clusters they are members of
        // This query fetches the user's membership records, specifically checking if the vcf has been downloaded.
        const { data: userDownloadRecords, error: downloadError } = await supabase
            .from('cluster_cohort_members')
            .select('cluster_id, vcf_downloaded_at')
            .eq('user_id', user_id);

        if (downloadError) throw downloadError;

        // Create a set of cluster_ids where the user has successfully downloaded the VCF
        const downloadedClusterIds = new Set(
            userDownloadRecords
                .filter(rec => !!rec.vcf_downloaded_at) // Only count if downloaded_at is NOT null
                .map(rec => rec.cluster_id)
        );

        // Step 3: Combine the full cluster list with the user's download status
        const clusters = allClusters.map(cluster => {
            return {
                ...cluster,
                // Check if the cluster_id exists in our set of downloaded IDs
                user_has_downloaded: downloadedClusterIds.has(cluster.cluster_id),
            };
        });
        ...
        console.error('SECURE GET CLUSTERS (Goal 1 Fix) ERROR:', e.message);
...
// ----------------------------------------------------
// API ENDPOINT: TRACK DOWNLOAD (Unchanged - Tracks aggregate count and user status)
// ----------------------------------------------------
