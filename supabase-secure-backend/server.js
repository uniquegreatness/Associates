...
// --- NEW HELPER FUNCTION: CLUSTER STATS CALCULATION ---

/**
 * Calculates aggregate statistics from the list of cohort members.
...
function calculateClusterStats(members) {
    if (!members || members.length === 0) {
        ...
        return { total_members: 0, avg_age: 0, country_distribution: {}, profession_distribution: {} };
    }
    ...
    return {
        ...
        profession_distribution: professionCounts,
    };
}
...
// ----------------------------------------------------
// NEW SECURE API ENDPOINT: GET CLUSTER STATS (THE MISSING ENDPOINT)
// ----------------------------------------------------
app.get('/api/cluster-stats', async (req, res) => {
    ...
    try {
        ...
        // 3. Fetch full user profiles (needed for stats like age, country, profession, and for the list itself)
        const { data: profiles, error: profilesError } = await supabase
            .from('user_profiles')
            .select('user_id, nickname, age, country, profession')
            .in('user_id', userIds);
