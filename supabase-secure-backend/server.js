...
function generateVcfContent(contacts) {
    ...
    contacts.forEach(contact => {
        ...
        // Use the full formattedName in the Structured Name (N) field for better compatibility.
        vcfString += `N:;${formattedName};;; \n`; 

        // Use TEL with X-WAID for WhatsApp compatibility
...
// --- HELPER FUNCTION: CLUSTER STATS CALCULATION ---
 ...
 */
function calculateClusterStats(members) {
    if (!members || members.length === 0) {
        // Return structured zero values for clean frontend handling
        return { total_members: 0, avg_age: 0, geographic_mix: {}, gender_mix: {}, profession_mix: {} };
    }
    ...
    for (const country in countryCounts) {
        const percentage = total_members > 0 ? (countryCounts[country] / total_members) * 100 : 0;
        geographic_mix[country] = Math.round(percentage); 
    }
...
// --- DATABASE SERVICE FUNCTION: GET COHORT STATUS (UPDATED FOR PAUSE STATE) ---

/**
 * Retrieves the current status (cohort_id, members, is_full) and membership for a cluster.
 * Includes vcf_file_name and logic to maintain the PAUSE STATE.
...
async function getCohortStatus(cluster_id, user_id) {
    ...
    try {
        ...
        if (target_cohort_id) {
            ...
            // Step 2: Check Membership and Count in the cluster_cohort_members table
...
// ----------------------------------------------------
// SECURE API ENDPOINT: GET COHORT STATUS
// ----------------------------------------------------
...
// ----------------------------------------------------
// NEW SECURE API ENDPOINT: GET CLUSTER STATS
// ----------------------------------------------------
app.get('/api/cluster-stats', async (req, res) => {
    const { cluster_id } = req.query;

    if (!cluster_id) {
        return res.status(400).json({ success: false, message: 'Missing cluster_id query parameter.' });
    }
    ...
    try {
        ...
        if (!activeCohortId) {
            return res.status(200).json({
                ...
                cohort_members: [],
                cluster_stats: calculateClusterStats([]) // Returns structured zeroed stats
            });
        }
        ...
        if (userIds.length === 0) {
             return res.status(200).json({
                ...
                cluster_name: clusterName,
                cohort_members: [],
                cluster_stats: calculateClusterStats([])
            });
        }

        // 3. Fetch full user profiles (including 'gender')
        const { data: profiles, error: profilesError } = await supabase
            .from('user_profiles')
            .select('user_id, nickname, age, country, profession, gender')
            .in('user_id', userIds);
        ...
        const calculatedStats = calculateClusterStats(combinedMembers);
...
// ----------------------------------------------------
// SECURE API ENDPOINT: JOIN CLUSTER (PAUSE STATE IMPLEMENTED)
// ----------------------------------------------------
app.post('/api/join-cluster', async (req, res) => {
    ...
    try {
        ...
        // Check if VCF is already uploaded (the PAUSE STATE) - Cannot join a full, paused cluster
        ...
        if (activeCohortId.startsWith('C_OPEN_')) {
            ...
            const { error: metaUpdateError } = await supabase
                ...
                .update({ 
                    ...
                    vcf_file_name: null // Ensure file name is clear for new cohort
                }) 
                .eq('cluster_id', clusterIdNum);
        ...
        if (isFullAfterJoin) {
            ...
            // --- 3d. CRITICAL: Update Metadata to PAUSE the cluster ---
            if (vcfUploadSuccessful) {
                ...
                const { error: statusUpdateError } = await supabase
                    ...
                    .update({ 
                        ...
                        vcf_file_name: null // Clear any potential stale file name
                    }) 
                    .eq('cluster_id', clusterIdNum); 
...
// ----------------------------------------------------
// END SECURE API ENDPOINT: JOIN CLUSTER
// ----------------------------------------------------
...
app.get('/api/download-contacts', async (req, res) => {
    // Expect file_name from the client
    ...
    // Use the specific file name in the storage path
    ...
    try {
        ...
        // Use the specific file name in the Content-Disposition header
...
// ----------------------------------------------------
// EXISTING ROUTES (Waitlist and Leaderboard)
// ----------------------------------------------------
