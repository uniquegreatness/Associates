...
// --- NEW HELPER FUNCTION: INJECT SUPABASE CONFIGURATION (Unchanged) ---

/**
 * Reads an HTML file, injects the Supabase URL and Anon Key into the global scope
 * of the client-side JavaScript, and sends the modified HTML.
 * @param {string} filePath - The path to the HTML file (e.g., 'cohort.html').
...
function injectSupabaseConfig(filePath, res) {
    ...
    fs.readFile(filePathFull, 'utf8', (err, html) => {
        if (err) {
            ...
            return res.status(500).send('Internal Server Error: Could not read HTML file.');
        }

        const injectedHtml = html
            .replace(/__supabase_url/g, JSON.stringify(supabaseUrl))
            .replace(/__supabase_anon_key/g, JSON.stringify(supabaseAnonKey));
...
// --- FRONTEND SERVING CONFIGURATION (MODIFIED) ---
...
// NOTE: The following specific routes are now handled by the static middleware *OR*
// they are handled by the injectSupabaseConfig function below, ensuring only pages 
// needing dynamic config use the slower read/replace method.

// Route for the login page
// app.get('/login.html', (req, res) => { res.sendFile(...) }); // Removed: Handled by static middleware

// Route for the waitlist page
// app.get('/newwaitlist.html', (req, res) => { res.sendFile(...) }); // Removed: Handled by static middleware

// Route for the leaderboard (secure dashboard) - USES INJECTION
app.get('/leaderboard.html', (req, res) => {
    // Requires Supabase client for session checks and data fetch
    injectSupabaseConfig('leaderboard.html', res);
});

// Route for the new cohort page (cohort.html) - USES INJECTION
app.get('/cohort.html', (req, res) => {
    // This is the page requiring Supabase config for auth
    injectSupabaseConfig('cohort.html', res);
});

// Dedicated page for password reset/update - USES INJECTION
app.get('/update-password.html', (req, res) => {
    // Requires Supabase client for auth context
    injectSupabaseConfig('update-password.html', res);
});

// ----------------------------------------------------
// SECURE API ENDPOINT: JOIN CLUSTER (RPC Call) - UNCHANGED
// ----------------------------------------------------
app.post('/api/join-cluster', async (req, res) => {
    ...
    try {
        ...
        const { data: result, error: rpcError } = await supabase.rpc('join_or_complete_cohort', {
            ...
            p_display_profession: display_profession
        });
