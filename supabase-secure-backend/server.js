...
// --- REWRITTEN DATABASE SERVICE FUNCTION: GET COHORT STATUS (Updated for vcf_downloads_count) ---
...
async function getCohortStatus(cluster_id, user_id) {
    ...
    try {
        ...
        const { data: clusterMeta, error: metaError } = await supabase
            ...
            // FIX: Changed select column from plural 'vcf_downloads_count' to singular 'vcf_download_count'
        ...
        // FIX: Retrieving from singular 'vcf_download_count'
...
app.post('/api/admin/clusters', requireAdminAuth, async (req, res) => {
    ...
    const clusterToInsert = {
        ...
        // FIX: Initializing with singular 'vcf_download_count'
        ...
        if (activeCohortId.startsWith('C_OPEN_')) {
            ...
            const { error: metaUpdateError } = await supabase
                ...
                .update({ 
                    ...
                    vcf_uploaded: false,
                    vcf_file_name: null,
                    // FIX: Resetting with singular 'vcf_download_count'
                    vcf_download_count: 0 
                }) 
                .eq('cluster_id', clusterIdNum);
            ...
            if (vcfUploadSuccessful) {
                ...
                const { error: statusUpdateError } = await supabase
                    ...
                    .update({ 
                        ...
                        // FIX: Initializing download count with singular 'vcf_download_count'
                        vcf_download_count: 0
                    }) 
                    .eq('cluster_id', clusterIdNum); 
...
// ----------------------------------------------------
// API ENDPOINT: DOWNLOAD VCF (Updated to increment count)
// ----------------------------------------------------
    ...
    try {
        ...
        if (clusterIdToUpdate) {
            ...
            const { data: current, error: fetchError } = await supabase
                .from('cluster_metadata')
                // FIX: Selecting from singular 'vcf_download_count'
            ...
            if (fetchError || !current) {
                ...
                // FIX: Reading from singular 'vcf_download_count'
                const newCount = (current.vcf_download_count || 0) + 1;
                
                // 2. Update count
                const { error: updateError } = await supabase
                    .from('cluster_metadata')
                    // FIX: Updating singular 'vcf_download_count'
