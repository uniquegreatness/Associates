...
app.post('/api/request-password-reset', async (req, res) => {
    ...
    try {
        ...
        const { error } = await supabaseAnon.auth.resetPasswordForEmail(email, {
            // NOTE: The URL must point to the page where the user can enter the new password.
            redirectTo: `${req.protocol}://${req.get('host')}/update-password.html` 
        });

        if (error) {
            console.error('Password Reset Request Error:', error.message);
            // Crucially, never reveal if the email exists for security reasons.
            // Fall through to the generic success message below
        }
...
// 3. UPDATE: Edit an Existing Cluster (The code block you requested!)
...
// 4. DELETE: Delete a Cluster (Renumbered from 3 to 4)
...
app.post('/api/waitlist', async (req, res) => {
    ...
    try {
        ...
        // *** REMOVED THE CUSTOM COOKIE LOGIC ***
        // const cookieOptions = { 
