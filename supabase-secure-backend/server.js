...
function calculateClusterStats(members, userCountry) {
    ...
    const genderCounts = {}; 
    const lookingForCounts = {}; // FIX: New counter for looking_for
    const availableForCounts = {}; // FIX: New counter for available_for
    ...
    members.forEach(member => {
        ...
        // FIX START: New Logic for Looking For Distribution
        const lookingFor = member.looking_for || '';
        // Assuming looking_for can be a comma-separated string, count each item
        if (lookingFor) {
            lookingFor.split(',').map(s => s.trim()).filter(s => s).forEach(item => {
                lookingForCounts[item] = (lookingForCounts[item] || 0) + 1;
            });
        }
        // FIX END

        // FIX START: New Logic for Available For Distribution
        const availableFor = member.available_for || '';
        // Assuming available_for can be a comma-separated string, count each item
        if (availableFor) {
            availableFor.split(',').map(s => s.trim()).filter(s => s).forEach(item => {
                availableForCounts[item] = (availableForCounts[item] || 0) + 1;
            });
        }
        // FIX END
    });
...
app.get('/api/cluster-stats', async (req, res) => {
    ...
    try {
        ...
        // 3. Fetch full user profiles (FIXED: Added 'gender', 'looking_for', and 'available_for' to the select query)
        const { data: profiles, error: profilesError } = await supabase
            .from('user_profiles')
            .select('user_id, nickname, age, country, profession, gender, looking_for, available_for') // <-- SURGICAL FIX HERE
            .in('user_id', userIds);
        ...
        const combinedMembers = cohortMembersRaw.map(memberRaw => {
            ...
            return {
                ...
                looking_for: profile?.looking_for || 'N/A', // <-- SURGICAL FIX HERE
                available_for: profile?.available_for || 'N/A', // <-- SURGICAL FIX HERE
