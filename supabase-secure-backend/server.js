...
// --- REWRITTEN DATABASE SERVICE FUNCTION: GET COHORT STATUS (Includes Synchronization Fix) ---
    ...
    try {
        ...
        if (target_cohort_id) {
            ...
            const { data: members, count: current_members_count, error: membersError } = await supabase
                ...
                .eq('cluster_id', cluster_id)
                .eq('cohort_id', target_cohort_cohort_id); 
...
app.get('/api/download-contacts', async (req, res) => {
    ...
    try {
        ...
        // --- LOGIC: Increment VCF Download Count ---
        const clusterIdToUpdate = extractClusterIdFromFileName(fileName);

        if (clusterIdToUpdate) {
            // Read-then-update logic (susceptible to race conditions, but best available client-side)
