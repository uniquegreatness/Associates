...
// --- NEW HELPER FUNCTION: CLUSTER STATS CALCULATION (FIXED) ---
...
function calculateClusterStats(members) {
    ...
    for (const country in countryCounts) {
        ...
        geographic_mix[country] = Math.round(percentage); // e.g., { "Nigeria": 100 }
    }
    
    const gender_mix = {};
    for (const gender in genderCounts) {
        const percentage = total_members > 0 ? (genderCounts[gender] / total_members) * 100 : 0;
        gender_mix[gender] = Math.round(percentage); // e.g., { "Male": 60, "Female": 40 }
    }
    ...
    for (const profession in professionCounts) {
        ...
        profession_mix[profession] = Math.round(percentage); // e.g., { "Engineer": 50, "Designer": 50 }
    }

    return {
        total_members,
        avg_age,
        geographic_mix, // <--- FIXED: Now returns actual country breakdown
        gender_mix,     // <--- ADDED: Gender breakdown
        profession_mix, // <--- FIXED: Profession breakdown (using the correct source)
    };
}
// --- END NEW HELPER FUNCTION ---


// --- REWRITTEN DATABASE SERVICE FUNCTION: GET COHORT STATUS (UPDATED FOR PAUSE STATE) ---
...
app.get('/api/cluster-stats', async (req, res) => {
    ...
    try {
        ...
        // 3. Fetch full user profiles (FIXED: Added 'gender' to the select query)
        ...
        const combinedMembers = cohortMembersRaw.map(memberRaw => {
            ...
            return {
                ...
                gender: profile?.gender || 'N/A', // <--- ADDED
