...
// --- NEW DATABASE SERVICE FUNCTION: GET COHORT STATUS ---

/**
 * Retrieves the current status (cohort_id, members, is_full) and membership for a cluster.
 * FIX: Re-engineered to query existing tables and correctly check user membership (Issues #2 & #3).
 * NOTE: This assumes 'cluster_metadata' has a 'vcf_uploaded' boolean field.
...
async function getCohortStatus(cluster_id, user_id) {
    ...
    try {
        ...
        // Step 1: Find the user's current cohort_id for this cluster_id from the membership table.
        // The previous code had the wrong table name ('cohort_members'), causing the "table not found" error.
        // We are using 'cluster_metadata' as per your confirmation, but please ensure this table 
        // actually stores the 'user_id' and 'cohort_id' for membership tracking.
        const { data: memberEntry, error: memberEntryError } = await supabase
            .from('cluster_metadata') // <-- FIX 1: Using 'cluster_metadata' instead of 'cohort_members'
            .select('cohort_id')
            .eq('user_id', user_id)
        ...
        if (memberEntry) {
            user_is_member = true;
            cohort_id = memberEntry.cohort_id;
        } 
        
        // Step 2: Determine the active cohort ID and check metadata status
        // This step already used 'cluster_metadata' correctly.
        const { data: clusterMeta, error: metaError } = await supabase
            .from('cluster_metadata') // <-- CORRECTED TABLE NAME (Fix for "table not found")
            .select('active_cohort_id, vcf_uploaded') // Assumes active_cohort_id and vcf_uploaded exist
            .eq('cluster_id', cluster_id)
            .limit(1)
            .maybeSingle();
        ...
        if (clusterMeta) {
            // If the user is a member, the status is determined by *their* cohort.
            // If not a member, the status is determined by the *active* cohort.
            const target_cohort_id = cohort_id || clusterMeta.active_cohort_id;
            vcf_uploaded = clusterMeta.vcf_uploaded || false; // Status for download (Issue #3 fix)
            
            if (target_cohort_id) {
                 // Now, count members in the target cohort.
                 // This query also needs the correct membership table name.
                const { count: current_members_count, error: countError } = await supabase
                    .from('cluster_metadata') // <-- FIX 2: Using 'cluster_metadata' instead of 'cohort_members'
            ...
            // Should not happen if all 10 clusters exist in metadata
            cohort_id = `C_OPEN_${cluster_id}`; 
            current_members = 0;
            is_full = false;
        }
        ...
        return {
            ...
            current_members,
            user_is_member,
            vcf_uploaded, // Include VCF status in the response
            message: "Cohort status retrieved successfully."
        };

    } catch (error) {
        console.error("Error in getCohortStatus:", error.message);
        return { success: false, message: `Database error: ${error.message}` };
    }
}

// --- END NEW DATABASE SERVICE FUNCTION ---
...
app.post('/api/join-cluster', async (req, res) => {
    ...
    try {
        ...
        if (cohortStatus.is_full) {
            ...
            // A Supabase RLS policy or view might be needed here, or we query two tables.
