...
function calculateClusterStats(members, userCountry) {
    ...
    members.forEach(member => {
        // Age calculation
        // CRITICAL FIX 1: Check if age is a valid number (it is now guaranteed to be a number or null from router)
        ...
        // CRITICAL FIX 2: Ensure profession is read as a safe string
        if (member.display_profession) {
            const profession = String(member.profession || 'Not Specified');
            professionCounts[profession] = (professionCounts[profession] || 0) + 1;
        }

        // Looking For Distribution (friend_reasons)
        // CRITICAL FIX 3: Process as a safe, comma-separated string (as passed by the router)
        const friendReasonsString = String(member.friend_reasons || '');
        if (friendReasonsString) {
            friendReasonsString.split(',').forEach(item => {
        ...
        // CRITICAL FIX 4: Process as a safe, comma-separated string (as passed by the router)
        const servicesString = String(member.services || '');
        if (servicesString) {
            servicesString.split(',').forEach(item => {
                const cleanItem = item ? item.trim() : '';
                if (cleanItem) {
...
async function getClusterCohortStatus(supabase, clusterId, userId) {
    ...
    // CRITICAL FIX 5: Calculate spots_left before returning
    status.spots_left = Math.max(0, status.max_members - status.current_members);
