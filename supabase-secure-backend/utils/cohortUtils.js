...
async function getClusterCohortStatus(supabase, clusterId, userId) {
    ...
    // 1. Get Cluster Configuration (to get max_members)
    ...
    if (clusterError) {
        console.error(`DB Error fetching dynamic cluster ${clusterId} config:`, clusterError);
    } else if (clusterData) {
    ...
    const { data: membershipData, error: membershipError } = await supabase
        ...
        .select('cohort_id, vcf_downloaded')
        .eq('user_id', userId)
    ...
    if (membershipData) {
        ...
        status.user_has_downloaded = membershipData.vcf_downloaded || false;

        // 3. CRITICAL FIX: Count members ONLY in the user's specific cohort
        ...
        // 4. Fetch VCF status and download count (from cohort status table)
        // Assuming 'cohort_vcf_status' table exists to track vcf status and download metrics
        const { data: vcfStatus, error: vcfError } = await supabase
            .from('cohort_vcf_status') 
            .select('vcf_uploaded, vcf_file_name, vcf_download_count')
            .eq('cohort_id', status.cohort_id)
            .single();

        if (vcfStatus) {
            status.vcf_uploaded = vcfStatus.vcf_uploaded;
            status.vcf_file_name = vcfStatus.vcf_file_name;
            status.vcf_download_count = vcfStatus.vcf_download_count || 0; 
        }
