...
const fs = require('fs');
const { supabaseUrl, supabaseAnonKey } = require('../config/supabase'); // Get config variables
 ...
 * @param {string} __dirname - The current directory name for path resolution.
 */
function injectSupabaseConfig(templatePath, res, currentDirname) {
    const filePathFull = path.join(currentDirname, '..', templatePath);
